...
export function PaymentConfirmationPanel({ orders }: PaymentConfirmationPanelProps) {
  ...
  if (orders.length === 0) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <p className="mt-2 text-sm text-gray-500">No hay comprobantes por verificar. ✅</p>
      </div>
    )
  }
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  {order.story_text && (
                    <p className="mt-1 line-clamp-2 text-xs text-gray-400">{order.story_text}</p>
                  )}
              ...
              <div className="mt-3 flex items-center gap-2">
                {isDone ? (
                  <span className="text-sm font-medium text-green-600">✅ Pago confirmado y canción entregada</span>
                ) : result?.error ? (
