...
| PRP-001† | `prp-001-whatsapp-leads-campaign.md` | leads · AdminView | leads, orders, rebuys, promotions_catalog | COMPLETADO ✓ | 2026-03-25 |
...
| PRP-010b | `prp-storage-management.md` | storage-management · `/api/storage/cleanup` | storage_config, storage_cleanup_log | COMPLETADO ✓ | 2026-03-19 |
...
| (s/n) | `prp-event-tracker.md` | event-tracker · `/api/events/track` | events, content_outcomes | COMPLETADO | 2026-05-13 |
| (JUDGE) | `prp-judge-strategy-bridge.md` | avatar-research | judge_rankings, judge_overrides, avatars, avatar_insights, proactive_insights, content_outcomes | COMPLETADO ✓ | 2026-05-16 |
...
## Quirks de numeración (deuda de documentación, no bloqueante)

- **PRP-001 duplicado**: `prp-orders-conversational-flow` y `prp-001-whatsapp-leads-campaign`
  ambos se titulan PRP-001. (Marcado † arriba.)
- **PRP-010 duplicado**: `prp-rebuy-campaign` (PRP-010) y `prp-storage-management` (titulado
  PRP-010, referido como PRP-010b en CLAUDE.md). Se conserva PRP-010b aquí para desambiguar.
- **Sin número**: `prp-event-tracker` y `prp-judge-strategy-bridge` no usan `PRP-NNN` en el título.
- No se renumeraron los archivos existentes para no romper referencias; corregir requiere una
  pasada dedicada si se desea numeración única.
